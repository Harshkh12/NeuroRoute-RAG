...
5. **Build Sparse Retriever Index**
   ```python
   from src.rag.sparse_retriever import sparse_retriever
   sparse_retriever.build_index(chunks)  # ✅ BM25 INDEX BUILT
   ```

6. **Store Chunks in Vector Database via retriever_chain()**
   ```python
   return retriever_chain(chunks)  # ✅ VECTOR STORE UPDATED
   ```
